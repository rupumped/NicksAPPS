...
parser.add_argument('src_channel', metavar='src', type=str, help='The URL of the channel from which you want to copy all messages')
parser.add_argument('dest_channel', metavar='dest', type=str, help='The URL of the channel to which you want to copy all messages')
...
source_team_id, source_channel_id = getTeamAndChannelID(args.src_channel)
destination_team_id, destination_channel_id = getTeamAndChannelID(args.dest_channel)

# Get messages from the source channel
response = requests.get(
	f'https://graph.microsoft.com/v1.0/teams/{source_team_id}/channels/{source_channel_id}/messages',
	headers=headers
)
